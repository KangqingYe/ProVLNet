...
class GlobalAveragePoolingHead(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)#[8,2048,12,12]->[8,256,3,3]

        batch_size, n_channels = x.shape[:2]
        x = x.view((batch_size, n_channels, -1))
        x = x.mean(dim=-1)

        out = self.head(x)#[8,256]->[8,17]
...
class PoseResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, "alg_confidences"):
            alg_confidences = self.alg_confidences(x)#[8,2048,12,12]
        ...
        return heatmaps, features, alg_confidences, vol_confidences#[bs*n_views,17,96,96],[bs*n_views,17,96,96],[bs*n_views,17]


def get_pose_net(config, device='cuda:0'):
    block_class, layers = resnet_spec[config.res_num_layers]

    model = PoseResNet(
        block_class, layers, config.num_classes,
        num_input_channels=3,
        deconv_with_bias=False,
        num_deconv_layers=3,
        num_deconv_filters=(256, 256, 256),
        num_deconv_kernels=(4, 4, 4),
        final_conv_kernel=1,
        alg_confidences=config.res_alg_confidences,
        vol_confidences=config.res_vol_confidences
    )

    if config.res_init_weights:
        print("Loading pretrained weights from: {}".format(config.res_checkpoint))
        model_state_dict = model.state_dict()
        pretrained_state_dict = torch.load(config.res_checkpoint, map_location=device)
