...
tv1, tv2 = torch.__version__.split('.')[:2]
...
class CamFusionModule(nn.Module):
    def __init__(self, nview, njoint, h, w):
        super().__init__()
        self.nview = nview
        # self.batch = batch
    ...
    def forward(self, heatmaps, affine_trans, cam_Intri, cam_R, cam_T, inv_affine_trans,standard_cam_T):
        dev = heatmaps.device
        batch = heatmaps.shape[0] // self.nview
        # pad_hmap = torch.zeros(batch*2, self.njoint, 192,64,device=dev)#padding to square #need to change with dataset
        # heatmaps = torch.cat((heatmaps, pad_hmap), dim=-1)
        self.onehm = self.onehm.to(dev)

        cam_Intri = cam_Intri.to(dev)
        cam_R = cam_R.to(dev)
        cam_T = cam_T.to(dev)
        standard_cam_T = standard_cam_T.to(dev)
        ...
        for dep in xview_self_depth:
            ...
            # x_g = torch.bmm(cam_R, dep_mul_factor * torch.bmm(inv_cam_Intri, torch.bmm(inv_affine_trans, uvhm_coords))) + cam_T
            x_g = torch.bmm(inv_cam_R, dep_mul_factor * torch.bmm(inv_cam_Intri, torch.bmm(inv_affine_trans, uvhm_coords))) + cam_T
        ...
        for curview in range(self.nview):
            ...
            for idx_othview, othview in enumerate(other_views):
                ...
                for idxpt in range(2):
                    ...
                    tmp_std_T = standard_cam_T[othview::self.nview]
                    tmp_Aff = affine_trans[othview::self.nview]

                    # tmp_P=torch.cat((tmp_R, tmp_std_T), dim=2)
                    # points = tmp_x_g
                    # tmp_x_g = torch.cat([points, torch.ones((points.shape[0], 1,points.shape[2]), dtype=points.dtype, device=points.device)], dim=1)
                    # xcam = torch.bmm(tmp_P, tmp_x_g)
                ...
                # x0 = ref_pt0[:,0].view(batch,1,-1).expand(-1,self.h,-1)
                # y0 = ref_pt0[:,1].view(batch,1,-1).expand(-1,self.h,-1)
            ...
            sample_hm = grid_sample(heatmaps_sample, grid, mode='bilinear')
        ...
        return crossview_fused#[:,:,:,:,:128]#need to change with dataset
